...
# Description: Script to create .yaml configuration file to run job_array with slurm for computing daily mean for surface level ERA5 data
#
######################################################################
...
date_lst = pd.date_range('2000-03-01', '2023-12-31', freq='1Y')
...
for i, njobs in enumerate(njob_lst):
    ...
    for j, job in enumerate(range(1, njobs+1, 1)):
        call_string = "python era5_1hr_to_daily_mean_sfc.py config_{0}.yaml 'job_{1}'".format(i+1, j+1)
