...
for i, region in enumerate(region_lst):
    for j, lag in enumerate(lag_lst):
        for k, ssn in enumerate(ssn_lst):
            for l, anom in enumerate(anom_lst):
                for m, varname in enumerate(varname_lst):
...
for i, njobs in enumerate(njob_lst):
    ...
    for j, job in enumerate(range(1, njobs+1, 1)):
        call_string = "python compute_horizontal_composites.py config_{0}.yaml 'job_{1}'".format(i+1, j+1)
