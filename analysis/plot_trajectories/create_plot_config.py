######################################################################
# Filename:    create_plot_configs.py
# Author:      Deanna Nash dnash@ucsd.edu
# Description: Script to create .yaml configuration file to create trajectory plots
#
######################################################################
...
ssn_lst = ['NDJFMA', 'MJJASO', 'DJF', 'MAM', 'JJA', 'SON']
ar_lst = [True, False]
...
dest_lst = []
njob_lst = []
for i, ARDT in enumerate(ARDT_lst):
    for j, ssn in enumerate(ssn_lst):
        for k, ar in enumerate(ar_lst):
            jobcounter += 1
...
for i, njobs in enumerate(njob_lst):
    ...
    for j, job in enumerate(range(1, njobs+1, 1)):
        call_string = "python plot_trajectories.py config_{0}.yaml 'job_{1}'".format(i+1, j+1)
